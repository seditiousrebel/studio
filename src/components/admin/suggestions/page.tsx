...
export default function AdminSuggestionsPage() {
  ...
  const fetchSuggestions = async () => {
    ...
    try {
      const response = await fetch(ROUTES.API.SUGGESTIONS);
      if (!response.ok) {
      ...
      if (selectedSuggestionForView && !selectedSuggestionForView.is_new_item_suggestion && selectedSuggestionForView.entity_id) {
        ...
        try {
          ...
          switch (selectedSuggestionForView.entity_type) {
            case 'politician': apiUrl = ROUTES.API.POLITICIAN_DETAIL(selectedSuggestionForView.entity_id); break;
            case 'party': apiUrl = ROUTES.API.PARTY_DETAIL(selectedSuggestionForView.entity_id); break;
            case 'promise': apiUrl = ROUTES.API.PROMISE_DETAIL(selectedSuggestionForView.entity_id); break;
            case 'bill': apiUrl = ROUTES.API.BILL_DETAIL(selectedSuggestionForView.entity_id); break;
            default:
  ...
  const renderSuggestedData = (suggestedData: EditSuggestion['suggestedData'], originalData: any | null, entityType: EntityType) => {
    ...
    return Object.entries(suggestedData).map(([key, value]) => {
      ...
      if (isEmptySuggestion && !(changeIndicator && String(changeIndicator).includes("Removed")) ) { 
        ...
        try {
          displayValue = formatDate(value as string); // Use formatDate
  ...
  const getEditSuggestionPath = (suggestion: EditSuggestion) => {
    return ROUTES.ADMIN.SUGGESTION_EDIT(suggestion.id, suggestion.entity_type);
  };
            ...
            <CardContent className="p-0">
                ...
                <TableBody>
                    ...
                    <TableRow key={suggestion.id}>
                        ...
                        <TableCell>{formatDate(suggestion.timestamp, 'N/A')}</TableCell>
                        ...
                        <div className="flex flex-col items-end gap-1 sm:flex-row sm:items-center sm:justify-end sm:gap-2">
                            ...
                            <Button asChild variant="outline" size="sm">
                                <Link href={getEditSuggestionPath(suggestion)} prefetch={true}><Edit2Icon className="mr-1 h-4 w-4" /> Edit</Link>
                            </Button>
        ...
        {selectedSuggestionForView && (
            ...
            <AlertDialogContent className="sm:max-w-lg md:max-w-2xl max-h-[80vh]">
                ...
                <AlertDialogDescription>
                    Review the details of this suggestion. Submitted by: {selectedSuggestionForView.user_name || selectedSuggestionForView.user_id} on {formatDate(selectedSuggestionForView.timestamp, 'N/A')}.
