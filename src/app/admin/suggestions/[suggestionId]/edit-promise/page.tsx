...
export default function AdminEditPromiseSuggestionPage() {
  const { supabase } = useAuth(); // Auth checks handled by AdminLayout
  ...
  const [existingPromiseTags, setExistingPromiseTags] = useState<string[]>([]);
  const [promiseDataForForm, setPromiseDataForForm] = useState<Partial<PromiseFormValues> | null>(null);
      ...
      const fetchData = async () => {
        ...
        try {
          ...
          if (suggestionError || !fetchedSuggestion) {
           ...
           if (fetchedSuggestion.entity_type !== 'promise') {
          ...
          setPromiseDataForForm(getPromiseFormInitialValues(fetchedSuggestion.suggested_data as Partial<UserPromise> | Partial<PromiseFormValues>));
          ...
          const { data: catsRows, error: catsError } = await supabase.from('promises').select('category');
          if (catsError) throw catsError;
          if (catsRows) setExistingCategories(Array.from(new Set(catsRows.map(p => p.category).filter(Boolean) as string[])).sort());

          const { data: tagLinks, error: tagsError } = await supabase.from('promise_tags').select('tags(name)');
          if (tagsError) throw tagsError;
          if (tagLinks) setExistingPromiseTags(Array.from(new Set(tagLinks.map(tl => tl.tags?.name).filter(Boolean) as string[])).sort());
  ...
  const handleSubmitPromiseSuggestionEdits = async (formValues: PromiseFormValues) => { 
    if (!suggestion) return;
    setIsSubmitting(true);
    
    const updatedSuggestedData: Partial<PromiseFormValues> = { ...formValues };
    if (Array.isArray(formValues.tags)) {
      updatedSuggestedData.tags = formValues.tags.map(t => (typeof t === 'object' && t.name ? t.name : String(t))).join(',');
    } else {
      updatedSuggestedData.tags = String(formValues.tags || '');
    }
    ...
    try {
      ...
      toast({
        ...
        description: "The suggested promise data has been modified.",
      });
      router.refresh(); 
      router.push('/admin/suggestions');
    } catch (error: any) {
  ...
  if (isLoadingData || isLoadingOptions || !suggestion || !promiseDataForForm) { 
    return (
      <AdminLayout pageTitle="Edit Promise Suggestion" pageDescription="Loading data...">
        <Container className="py-8 md:py-12"><PageHeader title="Edit Promise Suggestion" description="Loading..." /><Skeleton className="h-[calc(100vh-200px)] w-full" /></Container>
      </AdminLayout>
    );
  }
  ...
  return (
    <AdminLayout
      pageTitle={`Edit Suggestion for: ${suggestion.is_new_item_suggestion ? 'New Promise' : ((promiseDataForForm as PromiseFormValues).title || suggestion.entity_id)}`}
      pageDescription="Modify the suggested promise data. These changes will update the suggestion itself."
    >
      <Container className="py-8 md:py-12">
        <PageHeader
          title={`Edit Suggestion for: ${suggestion.is_new_item_suggestion ? 'New Promise' : ((promiseDataForForm as PromiseFormValues).title || suggestion.entity_id)}`}
          description="Modify the suggested promise data. These changes will update the suggestion itself."
        />
        <Card className="mt-6">
          <CardContent className="pt-6">
            <PromiseForm
              promise={promiseDataForForm} 
              onSubmitForm={(data) => handleSubmitPromiseSuggestionEdits(data as PromiseFormValues)}
