...
import type { Bill, EditSuggestion, Tag } from '@/types';
...
export default function AdminEditBillSuggestionPage() {
  ...
  const [existingBillTags, setExistingBillTags] = useState<string[]>([]);
  const [existingMinistries, setExistingMinistries] = useState<string[]>([]);
  const [billDataForForm, setBillDataForForm] = useState<Partial<BillFormValues> | null>(null);
    ...
    if (suggestionId) {
      const fetchData = async () => {
        ...
        try {
          ...
          if (fetchedSuggestion.entity_type !== 'bill') {
          ...
          setBillDataForForm(getBillFormInitialValues(fetchedSuggestion.suggested_data as Partial<Bill> | Partial<BillFormValues>));
          ...
          const { data: ministriesRows, error: ministriesError } = await supabase.from('bills').select('ministry');
          if (ministriesError) throw ministriesError;
          if (ministriesRows) setExistingMinistries(Array.from(new Set(ministriesRows.map(b => b.ministry).filter(Boolean) as string[])).sort());

          const { data: tagLinks, error: tagsError } = await supabase.from('bill_tags').select('tags(name)');
          if (tagsError) throw tagsError;
          if (tagLinks) setExistingBillTags(Array.from(new Set(tagLinks.map(tl => tl.tags?.name).filter(Boolean) as string[])).sort());
  ...
  const handleSubmitBillSuggestionEdits = async (formData: BillFormValues) => {
    if (!suggestion) return;
    setIsSubmitting(true);

    const updatedSuggestedData: Partial<BillFormValues> = { ...formData };
    ...
    try {
      ...
      toast({
        ...
        description: "The suggested bill data has been modified.",
      });
      router.refresh(); 
      router.push('/admin/suggestions');
    } catch (error: any) {
  ...
  if (isLoadingData || isLoadingOptions || !suggestion || !billDataForForm) { 
    return (
      <AdminLayout pageTitle="Edit Bill Suggestion" pageDescription="Loading data...">
        <Container className="py-8 md:py-12"><PageHeader title="Edit Bill Suggestion" description="Loading..." /><Skeleton className="h-[calc(100vh-200px)] w-full" /></Container>
      </AdminLayout>
    );
  }
  ...
  return (
    <AdminLayout
        pageTitle={`Edit Suggestion for: ${suggestion.is_new_item_suggestion ? 'New Bill' : ((billDataForForm as BillFormValues)?.title || suggestion.entity_id)}`}
        pageDescription="Modify the suggested bill data. These changes will update the suggestion itself."
    >
      <Container className="py-8 md:py-12">
        <PageHeader
          title={`Edit Suggestion for: ${suggestion.is_new_item_suggestion ? 'New Bill' : ((billDataForForm as BillFormValues)?.title || suggestion.entity_id)}`}
          description="Modify the suggested bill data. These changes will update the suggestion itself."
        />
        <Card className="mt-6">
          <CardContent className="pt-6">
            <BillForm
              bill={billDataForForm}
              onSubmitForm={handleSubmitBillSuggestionEdits}
